...
echo "
Usage: $0 [-h] -- program to show system information.
	-h 	Show help information
  	--host		Report only on the host name
    --memory 	Report only on system memory information
  	--ipconfig	Report only on IP information
    --printer 	Report only on printer information
  	--cpu 		Report only on CPU information
    --os 		Report only on OS information
  	--di
sk 		Report only on system disk information
